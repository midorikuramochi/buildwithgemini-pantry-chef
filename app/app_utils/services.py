...
def get_memory_service():
    ...
    if uri:
        if uri.startswith("agentengine://") and "/" not in uri.removeprefix("agentengine://"):
            mb_id = uri.removeprefix("agentengine://")
            uri = f"agentengine://projects/qwiklabs-gcp-03-894441c8585c/locations/us-east1/reasoningEngines/{mb_id}"
        return create_memory_service_from_options(
            base_dir=_AGENT_DIR, memory_service_uri=uri
        )
    mb_id = os.environ.get("MEMORY_BANK_ID") or "739628277865381888"
    try:
        ...
        return VertexAiMemoryBankService(
            project="qwiklabs-gcp-03-894441c8585c",
            location="us-east1",
