...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            # Unwrap nested envelope if present: {"kind": "data", "metadata": ..., "data": ...}
            if isinstance(data, dict) and "data" in data and ("metadata" in data or "kind" in data):
                inner_meta = data.get("metadata") or {}
                if isinstance(inner_meta, dict):
                    meta = {**inner_meta, **meta}
                data = data["data"]
            mime = meta.get("mimeType") if isinstance(meta, dict) else None
            if mime == _A2UI_MIME or (isinstance(data, dict) and any(k in data for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate"))):
